...
%ENRICO:
%I'm a bit confused by \eqref{eqOverlap}. Clearly lambda is no longer a measure of overlap percentage, as when lambda=0 we have identical strategy, but when lambda=1 we can have a variety of patterns depending on what you have at A_a and K_a. For example if you have A_a=3 and K_a=5, you will get A_b=6 and K_b=10, thus no overlap between the two strategies. But if you have A_a=1 and K_a=5, then you will have A_b=2, and K_b=10 which implies an almost complete overlap. Besides this approach does not hold the differences constant. I thought you did:

% \begin{equation}
%\begin{aligned}
%\label{eqOverlap}
%A_b = A_a + (1-\lambda A_a)(K_a-A_a)\\
%K_b = K_a + (1-\lambda A_a)(K_a-A_a)
%\end{aligned}
% \end{equation}

%which would instead give, with lambda=0 this time, A_a=3 K_a=5 A_b=5, K_b=7 for the first example, and  A_a=1 K_a=5 A_b=5, K_b=9 for the second. 
...
%Ok this reads more like my interpretation of lambda.. 
