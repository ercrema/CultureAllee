...
The last three decades have witnessed a steady growth of formal models of cultural evolution. Early works inspired by population biology \citep{cavallisforza_feldman_1981,boyd1985} have laid the foundation of a now mature trans-disciplinary field which unites the social and biological sciences \citep{mesoudi_etal_2006}. The rich cross-fertilisation between different subfields has promoted the designing of new analytical techniques as well as new uses for existing methods, but more crucially have prompted the development of theoretical models of how culture is acquired and transmitted between individuals. %Add reference?  
...
Several behavioural traits are expected to show such density dependence. For example, behaviour linked to the exploitation of some limited resources are expected to exhibit decline when the density exceeds a critical threshold (i.e carrying capacity). Thus novel optimal variants are likely to spread rapidly when the trait is rare, but as it approaches carrying capacity, the  beneficial effects will start to decline, the payoff will become smaller, and eventually rarer suboptimal variants might be preferred (see Lake and Crema \citeyear{lake_and_crema_2012} for an extensive analysis). 
